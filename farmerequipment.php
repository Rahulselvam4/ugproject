    ...
    if (isset($_POST['submit'])) {
        $eqid=$_POST['submit'];
        $select_cart = mysqli_query($conn, "select * from cartequipment where eqid = '$eqid'") or die('query failed');
        ...
        mysqli_query($conn, "insert into farmequipment(farmer_id,eqid,eqname,Mobile,Location,Amount,model,description,document,user_id,vnumber)values('$farmer_id','$eqid','$eqname ','$mobile','$Location','$Amount','$model','$Description','$document','$user_id','$vnumber')")or die("query wrong");

        mysqli_query($conn, "DELETE FROM cartequipment WHERE eqid = ('$eqid')")or die("query wrong");
        echo "<script>alert('Product added to cart'); window.location.href = 'farmequipment.php';</script>";
    };
    ?>
 ...
 <nav class="navbar navbar-expand-lg bg-primary navbar-dark shadow-sm py-3 py-lg-0 px-3 px-lg-5" id="home">
     ...
     <div class="product">
         <div class="box-container">
             <form action="" method="post">
                 <?php
                    $select = mysqli_query($conn, "select * from cartequipment");
     ...
     <button type="submit" value="<?php echo $row['eqid'];?>" name="submit" style="width: 120px; background-color:orange; border-radius: 10px;"><b>Rent</b></button>
